...
class MobipaidPaymentModuleFrontController extends ModuleFrontController
{
    public $ssl = true;
    ...
    public function initContent()
    {
        ...
        MobipaidApi::$isTestMode = $this->module->is_test_mode;
    ...
    private function getPaymentUrl($reference)
    {
        ...
        $messageLog = 'Mobipaid - postLink parameter '.json_encode($logData);
        $this->module->addPluginLogger($messageLog, 3, null, 'Cart', 0, true);

        $postLink = json_decode(MobipaidApi::generatePosLink($body), 1);

        $messageLog = 'Mobipaid - postLink  '.json_encode($postLink);
        $this->module->addPluginLogger($messageLog, 3, null, 'Cart', 0, true);
        ...
        if (isset($postLink['result']) && $postLink['result'] == 'success') {
            ...
            $this->module->addPluginLogger('Mobipaid - post link error currency', 3, null, 'Cart', $cart->id, true);
            $this->redirectError(
                $this->l('We are sorry, currency is not supported. Please contact us.')
            );
        } else {
            $this->module->addPluginLogger('Mobipaid - post link is not generated', 3, null, 'Cart', $cart->id, true);
            $this->redirectError(
     ...
     * @return string
     */
    private function getMobipaidCancelUrl()
    {
